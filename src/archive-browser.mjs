import {normalizeExport,orderedNodes,activePath,interactionInventory,candidateCourseConversation,safeConversationFilename} from "./archive-core.mjs";
let all=[],selected=null,showCode=false,filterCourse=true,query="";
...
export function mountArchive(host,onBack){
 ...
 const head=panel("Acervo privado — inspecionar conversas originais");
 head.append(dom("p","Selecione o conversations.json exportado da sua conta. O arquivo será lido somente neste navegador; não será enviado à rede, armazenado em localStorage ou incluído no GitHub.","notice"));
 ...
 const picker=dom("input");picker.type="file";picker.accept=".json,application/json";picker.setAttribute("aria-label","Selecionar conversations.json para inspeção local");
 picker.addEventListener("change",async()=>{
  ...
  try{
   ...
   const parsed=JSON.parse(await f.text());all=normalizeExport(parsed);renderIndex(host,onBack);
